...
function parseOperation(input, index) {
    if (input.op === 'upsert_node') {
        if (input.node === undefined)
            throw new GraphStoreError('invalid_input', `operations[${index}].node is required`);
        return { op: input.op, expectedVersion: input.expectedVersion, node: input.node };
    }
    if (input.op === 'delete_node') {
        if (input.nodeId === undefined)
            throw new GraphStoreError('invalid_input', `operations[${index}].nodeId is required`);
    ...
    if (input.op === 'upsert_edge') {
        if (input.edge === undefined)
            throw new GraphStoreError('invalid_input', `operations[${index}].edge is required`);
        return { op: input.op, expectedVersion: input.expectedVersion, edge: input.edge };
    }
    if (input.edgeId === undefined)
        throw new GraphStoreError('invalid_input', `operations[${index}].edgeId is required`);
    return { op: input.op, expectedVersion: input.expectedVersion, edgeId: input.edgeId };
}
export function createNovelGraphServer(store, onCommit) {
    ...
    server.registerTool('search_nodes', {
        ...
        try {
            const result = await store.searchNodes(await physicalNovelId(input.novelId), input.query, (input.kinds ?? []), input.limit ?? 20);
    ...
    server.registerTool('list_edges', {
        ...
        try {
            return success({
                ok: true,
                ...await store.listEdges(await physicalNovelId(input.novelId), input.nodeId, input.direction ?? 'both', input.types ?? [], input.limit ?? 50),
    ...
    server.registerTool('commit_changes', {
        ...
        try {
            ...
            await onCommit?.({ novelId: input.novelId, result: result });
            return success({ ok: true, ...result, novelId: input.novelId });
        }
        catch (error) {
            return failure(error);
