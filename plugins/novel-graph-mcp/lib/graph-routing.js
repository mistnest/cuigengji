...
export async function physicalNovelId(novelId, root = resultsRoot()) {
    ...
    try {
        ...
        if (typeof target !== 'string' || !valid.test(target))
            throw new GraphStoreError('corrupt_store', 'invalid graph catalog pointer');
        return target;
    }
    catch (error) {
        if (error.code === 'ENOENT')
            return routedNovelId;
        throw error;
    }
}
