...
test('publishes all ordered chapter handoffs in one graph transaction', async t => {
  const root=await mkdtemp(join(tmpdir(),'novel-chapter-memory-')); t.after(async()=>rm(root,{recursive:true,force:true}))
  const paths:string[]=[]
  for (const [index,addition] of ['第一次选择。','选择产生后果。'].entries()) {
    const path=join(root,`handoff-${index+1}.json`); paths.push(path)
  ...
  const first=await commitChapterMemory(options); const resumed=await commitChapterMemory(options)
  assert.deepEqual(resumed,first); assert.equal(first.graph_version,1); assert.equal(first.transaction_id === null,false); assert.equal(first.handoff_sha256s.length,2)
