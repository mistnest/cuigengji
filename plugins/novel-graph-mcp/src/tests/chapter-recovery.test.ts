...
import type { CommitRequest, GraphNodeInput } from '../types.js'

const node: GraphNodeInput = { id: 'a', kind: 'character_card', name: '林晚', summary: '出发', body: '保管信件', sourceRevisionId: 'r0' }
const edge = { id: 'zz-target', type: 'knows', fromNodeId: 'a', toNodeId: 'b', summary: '相识', body: '旧的关系事实', sourceRevisionId: 'r0' }
const initial: CommitRequest = { novelId: 'novel', requestId: 'initial', expectedGraphVersion: 0, operations: [
...
for (const snapshotInterval of [1, 100]) test(`request replay survives ${snapshotInterval === 1 ? 'snapshot' : 'journal'} reload and rejects changed payload`, async t => {
...
test('chapter merges nodes and edges beyond listing limit, restores lost receipt across MCP processes and snapshot', async t => {
  ...
  for (let batch = 0; batch < 3; batch++) {
    await store.commit({ novelId: 'novel', expectedGraphVersion: batch + 1, operations: Array.from({ length: 70 }, (_, i) => ({
      op: 'upsert_edge' as const, expectedVersion: 0, edge: { ...edge, id: `edge-${batch}-${i.toString().padStart(2, '0')}` },
    })) })
  }
...
test('a failed chapter transaction leaves neither partial nodes nor a successful request record', async t => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }))
  const invalid: CommitRequest = { ...initial, operations: [initial.operations[0]!, initial.operations[2]!] }
