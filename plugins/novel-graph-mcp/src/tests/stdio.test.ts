...
test('serves the five stable tools over real stdio and returns structured content', async t => {
  ...
  assert.equal((committed.structuredContent as { ok: boolean }).ok, true)

  const found = await client.callTool({
    name: 'search_nodes',
    arguments: { novelId: 'stdio-novel', query: '林晚' },
  })
  const structured = found.structuredContent as { ok: boolean; nodes: Array<Record<string, unknown>> }
