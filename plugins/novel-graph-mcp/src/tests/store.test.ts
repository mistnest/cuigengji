...
import type { GraphEdgeInput, GraphNodeInput } from '../types.js'

const linWan: GraphNodeInput = {
...
const relationship: GraphEdgeInput = {
...
async function temporaryStore(options: ConstructorParameters<typeof GraphStore>[1] = {}) {
...
test('rejects stale graph/entity versions and leaves the full batch unapplied', async t => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof GraphStoreError && error.code === 'version_conflict',
  )
...
test('uses tombstones and prevents active edges from pointing to deleted nodes', async t => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof GraphStoreError && error.code === 'referential_conflict',
  )
