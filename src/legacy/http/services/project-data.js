...
import {
    AppError,
    novelDir as resolveNovelDir,
} from '../../../backend/foundation/platform/index.js';
import { ensureChapterSummaries, ensureCharacterSummaries, ensureWorldBookSummaries, getChapterSummary } from '../../../backend/domains/knowledge/index.js';
...
async function readJson(filePath) {
    try {
        ...
        if (err instanceof SyntaxError) {
            throw new AppError(
                'CORRUPT_JSON',
                `Corrupt project data: ${path.basename(filePath)}`,
                { status: 500 },
            );
        }
        throw err;
