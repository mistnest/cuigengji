...
async function createProjectForDocument(filePath) {
    ...
    return createProject({ title: `${baseTitle}-${Date.now()}` });
}
    ...
    const handlers = new Map([
        [IMPORT_IPC_CHANNELS.selectDocument, createGuardedHandler(getMainWindow, async input => {
            ...
            const project = input?.projectId
                ? { id: input.projectId }
                : await createProjectForDocument(filePath);
        ...
        [IMPORT_IPC_CHANNELS.selectFolder, createGuardedHandler(getMainWindow, async input => {
            ...
            return folderPath ? importFolder(input?.projectId, folderPath) : null;
        ...
        [IMPORT_IPC_CHANNELS.selectWorldBook, createGuardedHandler(getMainWindow, async input => {
            ...
            return filePath ? importWorldBookFile(input?.projectId, filePath) : null;
        ...
        [IMPORT_IPC_CHANNELS.selectPreset, createGuardedHandler(getMainWindow, async input => {
            ...
            return filePath ? importPresetFile(input?.projectId, filePath) : null;
